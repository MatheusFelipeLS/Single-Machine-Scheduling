...
void attProductionInfo(const Data *data, ProductionInfo *s, int start) {
  ...
  for(i = start; i < (int) s->qtProductsWith0Fine+1; i++) {
...
bool sequenceIsBetter(const Data *data, ProductionInfo *s, int start, int stop) {
  ...
  if(!start) {
    ...
    currentTime = s->accumulatedTime[start-1] + data->switchTime(s->sequence[start-1], s->sequence[start]) + data->time(s->sequence[start]);
    currentPenalty = s->accumulatedFine[start-1] + max(0, data->penalty(s->sequence[start]) * (currentTime - data->deadline(s->sequence[start])));;
  }

  if(stop < s->qtProductsWith0Fine-1) stop++;
...
/*
OLHAR ISSO AQUI. NÃO TÁ ERRADO, MAS FAZ ALGO QUE PODE SER DIFERENTE
ESTÁ ANALISANDO A MULTA DE UM ELEMENTO COM MULTA 0, OU SEJA, SEM SENTIDO
*/
  for(int i = stop+1; i < s->qtProductsWith0Fine; i++) {
    s->accumulatedTime[i] = currentTime;
    s->accumulatedFine[i] = currentPenalty;
    currentTime += data->switchTime(s->sequence[i], s->sequence[i+1]) + data->time(s->sequence[i+1]);
    currentPenalty += max(0, data->penalty(s->sequence[i+1]) * (currentTime - data->deadline(s->sequence[i+1])));
    if(currentPenalty > s->accumulatedFine[i+1]) {
      return false;
    }
  }
...
ProductionInfo Guloso(const Data *data) {
  ProductionInfo s;
  s.qtProductsWith0Fine = data->getQtOrders();
  ...
  for(int i = 0; i < (int) data->getQtOrders(); i++) { 
    if(!data->penalty(i)) {
      s.qtProductsWith0Fine--;
      s.sequence[s.qtProductsWith0Fine] = i;
    } else {
      timeToInsert v;
      v.node = i;
      CL.push_back(v);
...
bool Swap(const Data*data, ProductionInfo *s) {
  ...
  int initialValue = s->accumulatedFine[s->qtProductsWith0Fine-1];

  for(int i = 0; i < s->qtProductsWith0Fine-1; i++) {
    for(int j = i+1; j < s->qtProductsWith0Fine; j++) {
      ...
      if(improved) {
        int delta = aux.accumulatedFine[aux.qtProductsWith0Fine-1] - initialValue;
        if(delta < bestDelta) {
          best_i = i;
          best_j = j;
          bestDelta = delta;
...
bool Rotate(const Data*data, ProductionInfo *s) {
  ...
  for(int i = 0; i < (int) s->qtProductsWith0Fine-1; i++) {
    for(int j = i+1; j < (int) s->qtProductsWith0Fine; j++) {
      ProductionInfo aux = *s;
...
bool Reinsertion(const Data*data, ProductionInfo *s, int range) {
  ...
  for(int i = 0; i < s->qtProductsWith0Fine-range+1; i++) {
    for(int j = i+range; j < s->qtProductsWith0Fine; j++) {
      ProductionInfo aux = *s;
...
void LocalSearch(const Data *data, ProductionInfo *s) {
  ...
  while(!n.empty()) {
    ...
    if(improved) {
      ...
      if(!s->accumulatedFine[s->qtProductsWith0Fine-1]) return;
      n = {1, 2, 3, 4, 5};
      // n = {3, 4, 5};
    } else {
      n.erase(n.begin()+x);
...
void Perturbacao(const Data *data, ProductionInfo *s) {
  int tamMax = (int) s->qtProductsWith0Fine / 10;

  int bloco1 = rand() % (tamMax -1) + 2;
  int bloco2 = rand() % (tamMax -1) + 2;

  int inicioDoBloco2 = (rand() % (s->qtProductsWith0Fine - bloco2 - bloco1-1)) + bloco1 +2;
...
Solution ILS(const Data *data, int max_iter) {
  ...
  for(int i = 0; i < max_iter; i++) {
    ...
    // cout << "custo após guloso: " << best.accumulatedFine[best.qtProductsWith0Fine-1] << endl;
    int iterILS = 0;
    while(iterILS <= maxIterIls) {
      LocalSearch(data, &s);
      // cout << "custo após busca local: " << s.accumulatedFine[s.qtProductsWith0Fine-1] << endl;
      if(s.accumulatedFine[s.qtProductsWith0Fine-1] < best.accumulatedFine[best.qtProductsWith0Fine-1]) {
        best = s;
        if(!s.accumulatedFine[s.qtProductsWith0Fine-1]) break;
        iterILS = 0;
      }
      Perturbacao(data, &s);
      // cout << "custo após perturbação: " << s.accumulatedFine[s.qtProductsWith0Fine-1] << endl;
      iterILS++;
    }

    if(best.accumulatedFine[best.qtProductsWith0Fine-1] < bestSolution.fine) {
      bestSolution.sequence = best.sequence;
      bestSolution.fine = best.accumulatedFine[best.qtProductsWith0Fine-1];
